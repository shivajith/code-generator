...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand('codegemma.generateCode', async () => {
    ...
    vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Querying CodeGen2-16B...', cancellable: false }, async () => {
      try {
        ...
        const doc = await vscode.workspace.openTextDocument({ content: result.generated_text, language: 'plaintext' });
        await vscode.window.showTextDocument(doc);
      } catch (err) {
        vscode.window.showErrorMessage('Error querying CodeGen2-16B: ' + err);
      }
    });
  });
